...
//		ALOGE("### buf %s, len :%d", buf, strlen(buf));
//		sprintf(mode, "%s", buf);
		if(len != 0) {
			if(!strcmp(buf, "On")) {
//				ALOGE("### touch mode On\n");
...
//				ALOGE("### touch mode Off\n");
			}
			else {
				ALOGE("### Error : Unknow touch mode");
			}
		}
		else {
			ALOGE("### Touch Mode read failed");
		}
		...
		if(fd != NULL) {
			fprintf(fd, "On");
//			ALOGE("Create touch_mode.dat\n"); 
			fclose(fd);
		}
		else {
			ALOGE("### Error : Create file -> %s\n", TOUCH_MODE_PATH);
			return 1;
		}
	}
