...
#if defined(CONFIG_PANTECH_PRESTO_BOARD) || defined(CONFIG_PANTECH_QUANTINA_BOARD)//p14682 kobj add 110720
#ifdef USE_3_6V_CUTOFF
#define SKY_SOC_EMPTY	36//32//28//32//56//36//56//8// 7 //20120319 PZ1949 from KBJ chagne p14682 kobj 110816 7 ->8
#define SKY_SOC_FULL	960//970//980 //992 // 20120319 PZ1949 from KBJ  chagne p14682 kobj 110816 992 ->989
#else
...
//20120319 PZ1949 from KBJ pz1946 20111007 rcomp change condition
...
#endif /* FEATURE_SKY_BATTERY_MAX17040 */
...
static void max17040_set_rcomp(void)
{
	int ret=0,i=0;
// paiksun...
#if defined(CONFIG_PANTECH_PRESTO_BOARD)			//p14682 kobj add 110720
...
static void max17040_get_soc(void)
{
	...
	int ret;
	int t_temp = 0;
	int t_rcomp = 0xe0;//20120517 PZ1949 //0x0e //20120319 PZ1949 from KBJ     //0xb0;			// ¢¯A¥ì¥ì ¨¬¢¬¡ío rcomp
	int f_rcomp;

	//printk("max17040_get_soc \n");
	t_temp = Presto_get_batt_temp();

	//printk("max17040_get_soc temp %d \n",t_temp);
	...
	//check i2c error
	if(msb<0 ||lsb <0||(msb>250&&lsb>250)) {
...
#if 1
		if(soc>=98)  //20120319 PZ1949 from KBJ
			soc=100;
#endif
	}
#endif
	...
	if(soc==0)
		//ps1 team shs : 0% persent is occured events
		max17040_data.event=Events;
...
static void max17040_work(struct work_struct *work)
{
	...
	//read voltage now
	max17040_get_vcell();
	max17040_get_soc(); //20120319 PZ1949 from KBJ
...
extern int Max_gpio_Sleep_set(int insleep); //20120319 PZ1949 from KBJ
		...
		pm_message_t state)
{
	...
	dbg_func_out();
	Max_gpio_Sleep_set(1);  //20120319 PZ1949 from KBJ
	return 0;
}
...
{
	dbg_func_in();
	Max_gpio_Sleep_set(0);	  //20120319 PZ1949 from KBJ
	if (max17040_data.slow_poll) {
		schedule_delayed_work(&max17040_data.work, 0);
